...
После прочтения [описания тестового задания](https://docs.google.com/document/d/1u-RQ5KCMiiICpnZUHaKV2IuE_KdX7aShbo3RmY6EpHI/edit) для меня было очевидно что есть две сущности, в моделях для таблиц я определили **предприятия** - *companies* а **продукты** - *products*. Также имеется в корневой директории проекта имеется файл `models.drawio` где можно увидеть ERD схему того как я понял описанное в ТЗ.
